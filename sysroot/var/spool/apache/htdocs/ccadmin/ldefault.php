  ...
  if ((isset($_SESSION['campid'])) && (!isset($_SESSION['listid']))) {
    ...
    <TR<?php print $bcolor[$rcnt % 2];$rcnt++;?>>
      <TH COLSPAN=3 CLASS=heading-body>
         <?php print _("Editing Default Fields For") . " " . $_SESSION['listname'] . " " . _("List For Campaign") . " " . $_SESSION['campname'];?>
      </TH>
    </TR><?php
...
?>
      <TR<?php print $bcolor[$rcnt % 2];$rcnt++;?>>
        <TD WIDTH=50%><?php print _($defname[$cnt]);?></TD>
        <TD>
           <SELECT NAME=<?php print $deffield[$cnt];?>><OPTION VALUE=""></OPTION><?php
             $tmp=$fdata;
