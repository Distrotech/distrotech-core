  ...
  <TR<?php print $bcolor[$rcnt % 2];$rcnt++;?>>
    ...
    <TR<?php print $bcolor[$rcnt % 2];$rcnt++;?>>
      <TH COLSPAN=3 CLASS=heading-body>
         <?php print _("Editing Fields For") . " " . $_SESSION['listname'] . " " . _("List For Campaign") . " " . $_SESSION['campname'];?>
      </TH>
    </TR><?php
    $testdbtbl=pg_query($db,$fieldnq);
    ...
    for($dtrcnt=0;$dtrcnt < pg_num_rows($testdbtbl);$dtrcnt++) {
      ...
      <TR<?php print $bcolor[$rcnt % 2];$rcnt++;?>>
        <TD>
          <INPUT TYPE=CHECKBOX NAME="del<?php print $trowid;?>">
        </TD>
        <TD>
          <?php print (($trown == "inputdata")?"Import Data (":"Script Data (") . $trowt . ")";?>
        </TD>
        <TD>
          <?php print $trows;?>
        </TD>
...
<?php    for($dtrcnt=0;$dtrcnt < pg_num_rows($testdbtbl);$dtrcnt++) {
