...
if (isset($pbxupdate)) {
  if ($newkey != "") {
    ...
    $eddi=pg_query($db,"SELECT id FROM astdb WHERE family='PDDI' AND key='" . $newkey . "'");
    if (pg_num_rows($eddi)) {
      $ddiid=pg_fetch_row($eddi,0);
      pg_query($db,"UPDATE astdb SET value='" . $newval . "' WHERE family='PDDI' AND key='" . $newkey . "'");
    } else {
      pg_query($db,"INSERT INTO astdb (family,key,value) VALUES ('PDDI','$newkey','$newval')");
    }
  } else if ($key != "") {
    pg_query($db,"DELETE FROM astdb WHERE family='PDDI' AND key='$key'");
    pg_query($db,"DELETE FROM astdb WHERE key='BGRP' AND family='$key'");
  }
}

$qgetdata=pg_query($db,"SELECT astdb.key,astdb.value,grp.value FROM astdb LEFT OUTER JOIN astdb AS grp ON (astdb.key=grp.family AND grp.key='BGRP') WHERE astdb.family='PDDI' ORDER BY astdb.key");


?>

<CENTER>
<FORM METHOD=POST NAME=ddipfwdf onsubmit="ajaxsubmit(this.name);return false">
<TABLE WIDTH=90% CELLPADDING=0 CELLSPACING=0>
<TR CLASS=list-color2>
  <TH CLASS=heading-body COLSPAN=2><?php print _("Asterisk PBX Port Forward Configuration");?></TH>
</TR>
<TR CLASS=list-color1>
<TD onmouseover="myHint.show('DA0')" onmouseout="myHint.hide()"><?php print _("Select Port Forward To Delete");?></TD>
<TD><SELECT NAME=key>
<OPTION VALUE=""><?php print _("Add New DDI Port Forward Below");?></OPTION>
<?php
...
<TR CLASS=list-color2>
<TD onmouseover="myHint.show('DA1')" onmouseout="myHint.hide()"><?php print _("Port To Forward") . " (I<PORT> = ISDN T<PORT> = TDM)";?></TD>
