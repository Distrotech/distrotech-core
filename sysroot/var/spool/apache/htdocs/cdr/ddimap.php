...
</FORM><FORM NAME=trunkfwdf METHOD=POST onsubmit="ajaxsubmit(this.name);return false">
...
print "<TR CLASS=list-color" . (($rcol % 2 )+1) . "><TH class=heading-body COLSPAN=" . $cspan . ">" . _("DDI Rewrite Map") . 
"</TH></TR>\n";
$rcol++;
...
print "<TH class=heading-body2>" . _("Match") . "</TH></TR>\n";
$rcol++;
...
if ($match != "") {
  ...
  pg_query($db,"INSERT INTO ddimap (prefix,strip,match) VALUES ('" . $prefix . "'," . $strip . ",'" . $match . "')");
}

$tmap=pg_query($db,"SELECT prefix,strip,match,id FROM ddimap ORDER by length(prefix)");

for($tcnt=0;$tcnt<pg_num_rows($tmap);$tcnt++) {
  $r=pg_fetch_array($tmap,$tcnt);
  $todel="del" . $r[3];
  if ($$todel == "on") {
    pg_query("DELETE FROM ddimap WHERE id=" . $r[3]);
    continue;
  }
