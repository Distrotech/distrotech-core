...
?>
<CENTER>
<FORM METHOD=POST NAME=dhcpdlog onsubmit="ajaxsubmit(this.name);return false">
<INPUT TYPE=HIDDEN NAME=classi VALUE="<?php print $euser;?>">
<TABLE WIDTH=90% cellspacing="0" cellpadding="0">
<TR CLASS=list-color2><TH CLASS=heading-body COLSPAN=2><?php print _("Last 100 DHCP Events");?></TH></TR>
<?php
  ...
  if ($ADMIN_USER == "admin") {
    ...
    $col++;
    $fp=popen("grep -E \"dhcpd: DHCP\" /var/log/messages |tail -100","r");
    ...
    while (!feof($fp)) {
      ...
      preg_match("/^([A-Za-z]+ [ 0-9][0-9] [0-9:]+).*dhcpd: (.*)/",$output,$data);
      if ($output != "") {
