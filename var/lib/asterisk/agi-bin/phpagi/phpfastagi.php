<?php
  ...
  */
  class AGI
  {
    ...
    */
    function AGI($socket,$config=NULL, $optconfig=array())
    {
      // load config
      ...
      // read the request
      $str = socket_read($this->socket,4096,PHP_NORMAL_READ);
      while($str != "\n")
      {
...
//        @syslog(LOG_WARNING, $msg);
    ...
    */
    function goto_pbx($context, $extension='s', $priority=1)
    {
    ...
    */
    function evaluate($command)
    {
      ...
      // write command
      if(@socket_write($this->socket, trim($command) . "\n") === false) return $broken;
      ...
      do
      {
        $str = trim(@socket_read($this->socket,4096,PHP_NORMAL_READ));
      } while($str == '' && $count++ < 5);
...
//        $this->conlog("evaluate error on read for $command");
      ...
      if($str{0} == '-') // we have a multiline response!
      {
        $count = 0;
        $str = substr($str, 1) . "\n";
        $line = socket_read($this->socket,4096,PHP_NORMAL_READ);
