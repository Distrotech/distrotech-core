...
#        open(NB,"/var/spool/apache/htdocs/statusnav.inc");
#        while (<NB>) {
#          print $_;
#        }
#        close(NB);
...
EOF
	...
	print $tgt->{pagefoot} if defined $tgt->{pagefoot};
        my (%form_data,$ban_image);
        my $query=new CGI;
        print $query->header;
        CGI::ReadParse(*form_data);

        if (-e "/var/spool/apache/htdocs/" . $form_data{'style'} . "/banner.png") {
          $ban_image=$form_data{'style'} . "/banner.png";
        } elsif (-e "/var/spool/apache/htdocs/banner.png") {
          $ban_image="banner.png";
        } else {
          $ban_image="digium-the-asterisk-company.gif";
        }
