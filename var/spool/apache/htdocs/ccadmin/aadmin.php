...
if ((!isset($_POST['id'])) && (!isset($_POST['listid'])) && (!isset($_POST['updb']))) {
  ...
  <TR<%print $bcolor[$rcnt % 2];$rcnt++;%>>
    <TH COLSPAN=2 CLASS=heading-body>
      <%print _("Select A Campaign To Assign To");%>
    </TH>
  </TR>
  <TR<%print $bcolor[$rcnt % 2];$rcnt++;%>>
    <TD WIDTH=50%>
      <%print _("Select Campaign To Configure Agents");%>
    </TD>
    <TD>
  ...
  if ((isset($_SESSION['campid'])) && (!isset($_SESSION['listid']))) {
    ...
    <TR<%print $bcolor[$rcnt % 2];$rcnt++;%>>
      <TH COLSPAN=2 CLASS=heading-body>
        <%print _("Select List To Assign Agents To") . " " . $_SESSION['campname'];%>
      </TH>
    </TR>
    <TR<%print $bcolor[$rcnt % 2];$rcnt++;%>>
      <TD WIDTH=50%>
    ...
    <TR<%print $bcolor[$rcnt % 2];$rcnt++;%>>
      <TH COLSPAN=2 CLASS=heading-body>
         <%print _("Assigning Agents To ") . " " . $_SESSION['listname'] . " " . _("List For Campaign") . " " . $_SESSION['campname'];%>
      </TH>
    </TR><%
    ...
    for($acnt=0;$acnt<pg_num_rows($cagentq);$acnt++) {
      $agdat=pg_fetch_array($cagentq,$acnt,PGSQL_NUM);%>
        <TR<%print $bcolor[$rcnt % 2];$rcnt++;%>>
          <TD>
            <INPUT TYPE=BUTTON VALUE="" CLASS="option-<%print ($agdat[0] == "t")?"green":"red";%>" onclick=ccagentonoff('<%print $agdat[2] . "','" . $agdat[0];%>')>
          </TD><TD>
            <%print $agdat[1];%>
          </TD>
          </TR><%
